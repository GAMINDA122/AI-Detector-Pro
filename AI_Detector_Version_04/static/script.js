...
document.getElementById('fileUpload').addEventListener('change', function() {
    ...
    Array.from(this.files).forEach(file => {
            ...
            reader.onload = function(e) {
                ...
                pdfjsLib.getDocument(typedarray).promise.then(pdf => {
                    ...
                    Promise.all(pagePromises).then(() => {
                        ...
                        const onlyLetters = pdfText.replace(/[^a-zA-Z\s]+/g, ' ').replace(/\s+/g, ' ').trim();
                        document.getElementById('contentText').value = onlyLetters;
                        document.getElementById('charCount').textContent = onlyLetters.length;
                    });
                });
            };
        ...
        // DOC/DOCX files
            ...
            reader.onload = function(e) {
                ...
                mammoth.extractRawText({ arrayBuffer })
                    .then(result => {
                        const onlyLetters = result.value.replace(/[^a-zA-Z\s]+/g, ' ').replace(/\s+/g, ' ').trim();
                        document.getElementById('contentText').value = onlyLetters;
                        document.getElementById('charCount').textContent = onlyLetters.length;
                    })
        ...
        // TXT and other text files
            ...
            reader.onload = function(e) {
                const onlyLetters = e.target.result.replace(/[^a-zA-Z\s]+/g, ' ').replace(/\s+/g, ' ').trim();
                document.getElementById('contentText').value = onlyLetters;
                document.getElementById('charCount').textContent = onlyLetters.length;
            };
...
document.getElementById('downloadBtn').addEventListener('click', function() {
    ...
    setTimeout(() => {
        try {
            ...
            doc.text(`AI Generated: ${currentAnalysisData.breakdown.ai_generated}%`, 25, 85);
            
            // Model Detection Section
            doc.setFontSize(14);
            doc.text('MODEL DETECTION', 20, 115);
            
            doc.setFontSize(11);
            doc.text(`GPT-4: ${currentAnalysisData.model_detection.chatgpt}%`, 25, 130);
            doc.text(`Claude-3: ${currentAnalysisData.model_detection.claude}%`, 25, 140);
            doc.text(`Gemini Pro: ${currentAnalysisData.model_detection.gemini}%`, 25, 150);
            
            // Content Type
            doc.setFontSize(14);
            doc.text('CONTENT TYPE', 20, 170);
            doc.setFontSize(11);
            doc.text(currentAnalysisData.content_type.toUpperCase(), 25, 185);
            
            // Analyzed Text Section
            doc.setFontSize(14);
            doc.text('ANALYZED TEXT', 20, 205);
            ...
            for (let i = 1; i <= pageCount; i++) {
                ...
                doc.text('Report generated by AI Content Detector Pro (Powered by Gemini AI)', 20, 275);
                doc.text(`Page ${i} of ${pageCount}`, 170, 275);
            }

...
class AIDetector:
    ...
    def dictionary_usage(self, text):
        ...
        slang_indicators = ['gonna', 'wanna', 'yeah', 'nah', 'cool', 'awesome']
    ...
    def predict_overall_probability(self, features, text):
        try:
            prompt = f"""Analyze this text and determine if it's AI-generated or human-written. Consider:
            ...
            ai_indicators = [
                features['perplexity'] > 60,    # AI → structured
                20 <= features['burstiness'] <= 60,    # AI → less variation 
                features['repetition'] < 10,    # AI → more repetition sometimes
                features['typo_score'] <= 2,    # AI → very few typos
                features['dictionary']['formal'] > 20   # AI → more formal words
            ]

            # New human indicator → if too many typos, more likely human
    ...
    def highlight_sentences(self, text, overall_probability):
        ...
        for sentence in sentences:
            ...
            if calibrated_sentence_prob >= 80:   # Threshold for strong AI detection
    ...
    def analyze_text(self, text, content_type='general'):
        ...
        if raw_ai_prob > 15.0:
            calibrated_ai_prob = raw_ai_prob - 15.0
        else:
            calibrated_ai_prob = 0.0
    
        calibrated_ai_prob = round(calibrated_ai_prob, 1)
